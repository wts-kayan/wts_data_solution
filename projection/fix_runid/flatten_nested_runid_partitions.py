...
# partCol.0 is `runId` (see recreate_table_partcol_runid.py) then the
# directories should be runId= too.
PARTITION_KEY    = "runId"
PARTITION_KEY_LC = PARTITION_KEY.lower()   # what the metastore stores
...
def path_only(uri):
    ...
    never recognised as living under a table root written file:///x, and the
    partition is misclassified as OUTSIDE the table root."""
    uri = str(uri).rstrip("/")
    i = uri.find("://")
    if i >= 0:
        j = uri.find("/", i + 3)
...
for st in root_entries:
    ...
    nested_part_dirs = [c for c in child_dirs
                        if split_key(pname(c.getPath()))[0].lower() == PARTITION_KEY_LC]
    ...
    # Case-insensitive on purpose: runId=<X>/runid=<X> is exactly the defect,
    # so the inner dir may legitimately differ from the canonical key in case.
    if inner_key.lower() != PARTITION_KEY_LC:
...
if METASTORE_PREFLIGHT and not table_exists(HIVE_TABLE):
    ...
    try:
        ...
        for spec in specs:
            ...
            elif len(rel) == 1 and rel[0].lower() == ("%s=%s" % (PARTITION_KEY, pval)).lower():
...
if EMIT_HIVE_DDL:
    ...
    for value in sorted(set(flattened)):
        ...
        ddl_statements.append(
            ...
            % (tag, HIVE_TABLE, PARTITION_KEY_LC, value))
        ddl_statements.append(
            "ALTER TABLE %s\n  ADD IF NOT EXISTS PARTITION (%s='%s')\n  LOCATION '%s';"
            % (HIVE_TABLE, PARTITION_KEY_LC, value, loc))
    ...
    for full, value, kind in unregistered:
        if kind == "canonical dir":
            commented.append(
                ...
                % (HIVE_TABLE, PARTITION_KEY_LC, value, full))
