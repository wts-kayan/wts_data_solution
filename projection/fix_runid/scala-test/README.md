...
| fix-all (batch) | only tables partitioned by `runid` are in scope; dry run renames and recreates nothing; both phases run over every in-scope table; a nested table is skipped by both phases and reported; a second run is a clean no-op; `ONLY_TABLES` limits the batch; the batch lands where the two single-table cells land |
...
All 39 tests run on Windows. Two things had to be arranged for that, both
