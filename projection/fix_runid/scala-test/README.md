...
| fix-all (batch) | a nested table is flattened and fully fixed in one run; a stray folder inside a partition dir is cleared for every table; a different run id is refused and blocks the recreate; `DO_FLATTEN=false` leaves the nesting and still blocks the recreate; dry run promotes nothing; only tables partitioned by `runid` are in scope; dry run renames and recreates nothing; both phases run over every in-scope table; a nested table is skipped by both phases and reported; `DO_RENAME=false` still refuses to recreate a nested table; a `purge=true` table is fixed rather than skipped and its data survives; purge is set false *before* the first drop is issued; an out-of-scope table keeps even its purge flag; a table already all `runId=` on disk is left completely alone; an already-correct table with a stale `partCol.0` is reported; one wrong-case dir among correct ones puts the table in scope; a nesting under a correct-case parent is still caught; a second run is a clean no-op; `ONLY_TABLES` limits the batch; the batch lands where the two single-table cells land |
...
All 62 tests run on Windows. Two things had to be arranged for that, both
