...
// imports not visible in screenshots

@Component(StepConstants.CRR3_IRB_COMPUTE_RWA)
@Scope(value = ConfigurableBeanFactory.SCOPE_PROTOTYPE)
public class StepCrr3IrbRwaCompute extends AbstractStepMap {

    @Value(PlaceholderConfigurerSupport.DEFAULT_PLACEHOLDER_PREFIX
            + ConfigurationConstants.PROPERTY_USE_CRR3_OPTIONS
            + PlaceholderConfigurerSupport.DEFAULT_PLACEHOLDER_SUFFIX)
    private boolean useCrr3;

    final double RATIO_CAPITAL_TO_RWA = 12.5;

    @Override
    public Facility execute(Facility facility) throws SparkException {
        //if(!useCrr3) return facility;

        Map<Rating, MeasurementsOfRating> facilityMeasurementsByRating =
                facility.getMeasurement().getMeasurementsByRating();

        if (facility.getApproche_bale_iv_rwa() != null
                && facility.getApproche_bale_iv_rwa() != Crr3Approach.IRBA
                && facility.getApproche_bale_iv_rwa() != Crr3Approach.IRBF) {
            return facility;
        }

        if (facilityMeasurementsByRating.values().stream()
                .noneMatch(measurementsOfRating ->
                        (measurementsOfRating.getUnpaid() != null
                                && measurementsOfRating.getUnpaid().isClean()))) {
            return facility;
        }
        ...
        if (facility.getApproche_bale_iv_rwa() == Crr3Approach.IRBA) {
            ...
            secured_rw = facility.getCrr3_irba_secured_rw();
        } else {
            unsecured_ead_ratio = facility.getCrr3_irbf_per_unsecured_ead();
            secured_ead_ratio = facility.getCrr3_irbf_secured_ead_ratio();
            secured_rw = facility.getCrr3_irbf_secured_rw();
        }
        ...
        for (Map.Entry<Rating, MeasurementsOfRating> mapEntry
                : facilityMeasurementsByRating.entrySet()) {

            final MeasurementsOfRating measurementsOfRating = mapEntry.getValue();
            ...
            if (unsecured_ead_ratio == 0.0
                    || measurementsOfRating.getEadReg() == 0) {
                ...
                rwa_unsecured =
                        measurementsOfRating.getCrr3_irb_partial_unsecured_capital()
                                * RATIO_CAPITAL_TO_RWA;

                measurementsOfRating.setCrr3_irb_unsecured_rwa(rwa_unsecured);

                rw_unsecured =
                        rwa_unsecured
                                / (measurementsOfRating.getEadReg()
                                * unsecured_ead_ratio);
            ...
            if (secured_ead_ratio == 0.0
                ...
                double rwa_tmp_secured =
                        measurementsOfRating.getCrr3_irb_partial_secured_capital()
                                * RATIO_CAPITAL_TO_RWA;
        ...
        facility.setCrr3_irb_secured_capital(
                rwa_secured / RATIO_CAPITAL_TO_RWA
        );

        facility.setCrr3_irb_unsecured_capital(
                rwa_unsecured / RATIO_CAPITAL_TO_RWA
        );

        facility.setCrr3_irb_capital(
                facility.getCrr3_irb_unsecured_capital()
                        + facility.getCrr3_irb_secured_capital()
        );
