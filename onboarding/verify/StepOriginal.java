...
/**
 * The body of StepCrr3IrbRwaCompute.execute() as it is on develop today,
 * copied verbatim from onboarding/StepCrr3IrbRwaCompute.java. Only the Spring
 * annotations, the SparkException and the useCrr3 field are dropped -- none of
 * them affects the computation.
 *
 * Do not tidy this file. It is the reference the refactoring is measured
 * against, so it has to keep the original's warts: entrySet(), the dead `ead`
 * loop, FastMath, the non-final constant, the five map lookups.
 */
final class StepOriginal {
